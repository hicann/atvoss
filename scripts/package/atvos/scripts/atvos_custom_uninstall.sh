...
if [ -z "$atvos_version_dir" ]; then
    ...
    logandprint "[INFO]: Start remove atvos softlinks."
    remove_atvos_include_softlink ${install_path}/${version_dir}
    if [ $? -ne 0 ]; then
        logandprint "[WARNING]: Remove atvos softlinks failed, some softlinks may not exist."
    else
        logandprint "[INFO]: Remove atvos softlinks successfully."
    fi
fi
