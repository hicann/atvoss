#!/bin/sh
# Perform upgrade for atvoss package
# -----------------------------------------------------------------------------------------------------------
...
install_info="${common_parse_dir}/share/info/atvoss/ascend_install.info"
if [ -f "$install_info" ]; then
    chip_type=$(get_install_param "Atvoss_Chip_Type" "${install_info}")
    feature_type=$(get_install_param "Atvoss_Feature_Type" "${install_info}")
fi
...
output_progress() {
    new_echo "INFO" "atvoss upgrade upgradePercentage:$1%"
    log "INFO" "atvoss upgrade upgradePercentage:$1%"
}
...
##########################################################################
log "INFO" "step into run_atvoss_upgrade.sh ......"
...
new_upgrade() {
    if [ ! -d "${sourcedir}" ]; then
        log "INFO" "no need to upgrade atvoss files."
        return 0
    fi
    output_progress 10
    ...
    sh "$curpath/install_common_parser.sh" --package="atvoss" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall --upgrade \
