#!/bin/sh
# Perform install/upgrade/uninstall for atvoss package
# -----------------------------------------------------------------------------------------------------------
...
atvoss_func_path="${curpath}/atvoss_func.sh"
...
update_install_param() {
    ...
    local install_info_key_array="Atvoss_Install_Type Atvoss_Chip_Type Atvoss_Feature_Type Atvoss_UserName Atvoss_UserGroup Atvoss_Install_Path_Param Atvoss_Arch_Linux_Path Atvoss_Hetero_Arch_Flag"
...
update_install_info_feature() {
    ...
    if [ "$featuremode" = "all" ] || [ "$operation" = "Upgrade" ]; then
        update_install_param "Atvoss_Feature_Type" "$featuremode" "$install_info"
        return
    fi
    local current_featuremode=$(get_install_param "Atvoss_Feature_Type" "$install_info")
...
update_install_info() {
    ...
    update_install_param "Atvoss_Install_Type" "$installmode" "$install_info"
    update_install_param "Atvoss_Chip_Type" "$chipmode" "$install_info"
    update_install_info_feature
    update_install_param "Atvoss_UserName" "$username" "$install_info"
    update_install_param "Atvoss_UserGroup" "$usergroup" "$install_info"
    update_install_param "Atvoss_Install_Path_Param" "$install_path_param" "$install_info"
    update_install_param "Atvoss_Arch_Linux_Path" "$arch_linux_path" "$install_info"
    update_install_param "Atvoss_Hetero_Arch_Flag" "$hetero_arch" "$install_info"
}
...
install_run() {
    ...
    local atvoss_install_path_param=""
    update_install_path
    update_install_info
    local atvoss_input_install_path=$(get_install_param "Atvoss_Install_Path_Param" "${install_info}")
    local atvoss_install_type=$(get_install_param "Atvoss_Install_Type" "${install_info}")
    if [ "$is_docker_install" = "y" ]; then
        atvoss_install_path_param=$(concat_docker_install_path "${docker_root}" "${atvoss_input_install_path}")
    else
        atvoss_install_path_param="${atvoss_input_install_path}"
    fi
    if [ "$1" = "install" ]; then
        unchattr_files
        chmod_start
        new_echo "INFO" "install ${atvoss_install_path_param} ${atvoss_install_type}"
        log "INFO" "install ${atvoss_install_path_param} ${atvoss_install_type}"
        sh "${curpath}/run_atvoss_install.sh" "install" "${atvoss_input_install_path}" "${atvoss_install_type}" \
            "${is_quiet}" "${input_setenv}" "${docker_root}" "${in_install_for_all}"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "Atvoss package installed successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
        else
            chmod_end
            log "ERROR" "Atvoss package install failed, please retry after uninstall!"
...
upgrade_run() {
    ...
    if [ -f "$install_info" ]; then
        local atvoss_input_install_path=$(get_install_param "Atvoss_Install_Path_Param" "${install_info}")
        local atvoss_install_type=$(get_install_param "Atvoss_Install_Type" "${install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i atvoss_install_path_param "$install_info_old") -ne 0 ]; then
        local atvoss_input_install_path="$(grep -iw atvoss_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local atvoss_install_type="$(grep -iw atvoss_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ "$1" = "upgrade" ]; then
        chmod_start
        new_echo "INFO" "upgrade ${atvoss_install_path_param} ${atvoss_install_type}"
        log "INFO" "upgrade ${atvoss_install_path_param} ${atvoss_install_type}"
        sh "${curpath}/run_atvoss_upgrade.sh" "upgrade" "${atvoss_input_install_path}" "${atvoss_install_type}" \
            "${is_quiet}" "${input_setenv}" "${docker_root}" "${in_install_for_all}"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "Atvoss package upgraded successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
        else
            chmod_end
            log "ERROR" "Atvoss package upgrade failed, please retry after uninstall!"
            log_operation "${operation}" "failed!"
            exit_install_log 1
        fi
    fi
...
uninstall_run() {
    ...
    if [ -f "$upgrade_install_info" ]; then
        local atvoss_input_install_path=$(get_install_param "Atvoss_Install_Path_Param" "${upgrade_install_info}")
        local atvoss_install_type=$(get_install_param "Atvoss_Install_Type" "${upgrade_install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i atvoss_install_path_param "$install_info_old") -ne 0 ]; then
        local atvoss_input_install_path="$(grep -iw atvoss_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local atvoss_install_type="$(grep -iw atvoss_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ ! -f "$upgrade_default_dir/script/run_atvoss_uninstall.sh" ]; then
        log "WARNING" "run_atvoss_uninstall.sh not found."
        return $?
    fi
    if [ "$1" = "uninstall" ]; then
        chmod_start "$upgrade_default_dir"
        new_echo "INFO" "uninstall ${atvoss_install_path_param} ${atvoss_install_type}"
        log "INFO" "uninstall ${atvoss_install_path_param} ${atvoss_install_type}"
        sh "$upgrade_default_dir/script/run_atvoss_uninstall.sh" "uninstall" "${atvoss_input_install_path}" "${atvoss_install_type}" "${is_quiet}" \
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $(id -u) -eq 0 ]; then
                if [ "$uninstall" = "y" ] && [ -f "$install_info_old" ] && [ $(grep -c -i atvoss_install_path_param "$install_info_old") -ne 0 ]; then
                    sed -i '/atvoss_install_path_param=/Id' "$install_info_old" 2> /dev/null
                    sed -i '/atvoss_install_type=/Id' "$install_info_old" 2> /dev/null
                fi
            fi
            new_echo "INFO" "Atvoss package uninstalled successfully! Uninstallation takes effect immediately."
            log "INFO" "Atvoss package uninstalled successfully! Uninstallation takes effect immediately."
            log_operation "${operation}" "succeeded"
        else
            log "ERROR" "Atvoss package uninstall failed!"
            log_operation "${operation}" "failed!"
            exit_uninstall_log 1
        fi
    fi
...
judgmentpath() {
    ...
    if [ $? -ne 0 ]; then
        log "ERROR" "The Atvoss install_path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
        exit 1
    fi
}
...
if [ "$featuremode" != "all" ]; then
    ...
    if [ "$ret" = "false" ]; then
        log "WARNING" "Atvoss package doesn't contain features $featuremode, skip installation."
        exit 0
    fi
fi
...
if [ "$pkg_is_multi_version" = "true" ] && [ "$hetero_arch" != "y" ]; then
    default_dir="${pkg_install_path}/$pkg_version_dir/share/info/atvoss"
else
    default_dir="${pkg_install_path}/share/info/atvoss"
fi
...
if [ "$input_pre_check" = "y" ]; then
    ...
    pre_check
    if [ $? -ne 0 ]; then
        log "WARNING" "Atvoss do pre check failed."
    else
...
if [ "$check" = "y" ]; then
    ...
    if [ -z "$pkg_version_dir" ]; then
        preinstall_check --install-path="$install_path_param" --script-dir="$curpath" --package="atvoss" --logfile="$logfile" --docker-root="$docker_root"
        if [ $? -ne 0 ]; then
            exit_install_log 1
        else
    ...
    ver_check
    if [ -z "$pkg_version_dir" ]; then
        preinstall_process --install-path="$install_path_param" --script-dir="$curpath" --package="atvoss" --logfile="$logfile" --docker-root="$docker_root"
        if [ $? -ne 0 ]; then
            exit_install_log 1
        else
            log "INFO" "version compatibility check successfully!"
...
uninstall_none_multi_version "$pkg_install_path/share/info/atvoss"
check_install_for_all
create_default_install_dir_for_common_user
log_base_version
...
if [ "x$version_installed" != "x" -a "$version_installed" != "none" ] || [ -f "${install_info}" ]; then
    ...
    # 升级场景
        ...
        if [ -n "$pkg_version_dir" ]; then
            ...
            # else
            #     export USE_SHARE_INFO="y"
            #     get_package_upgrade_install_info "upgrade_install_info" "$pkg_install_path" "atvoss"
            #     unset USE_SHARE_INFO
            # fi
    ...
    # 安装场景
        ...
        if [ "$is_quiet" = "n" ]; then
            log "INFO" "Atvoss package has been installed on the path ${pkg_install_path}, the version is ${version_installed}, and the version of this package is ${version_of_package}, do you want to continue? [y/n]"
            while true
            do
                read yn
                if test "$yn" = n; then
