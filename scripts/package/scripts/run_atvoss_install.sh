#!/bin/sh
# Perform install for atvoss package
# -----------------------------------------------------------------------------------------------------------
...
output_progress() {
    new_echo "INFO" "atvoss install upgradePercentage:$1%"
    log "INFO" "atvoss install upgradePercentage:$1%"
}
...
##########################################################################
log "INFO" "step into run_atvoss_install.sh ......"
...
new_install() {
    # if [ ! -d "${sourcedir}" ]; then
    #     log "INFO" "no need to install atvoss files."
    #     return 0
    # fi
    output_progress 10
    ...
    sh "$curpath/install_common_parser.sh" --package="atvoss" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall \
