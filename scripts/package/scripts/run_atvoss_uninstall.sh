...
SOURCE_INSTALL_COMMON_PARSER_FILE="${common_parse_dir}/share/info/atvoss/script/install_common_parser.sh"
SOURCE_FILELIST_FILE="${common_parse_dir}/share/info/atvoss/script/filelist.csv"
...
##########################################################################
log "INFO" "step into run_atvoss_uninstall.sh ......"
log "INFO" "uninstall target dir $common_parse_dir, type $common_parse_type."

if [ ! -d "$common_parse_dir/share/info/atvoss" ]; then
    log "ERROR" "ERR_NO:0x0001;ERR_DES:path $common_parse_dir/share/info/atvoss is not exist."
    exit 1
fi

new_uninstall() {
    if [ ! -d "${common_parse_dir}/share/info/atvoss" ]; then
        log "INFO" "no need to uninstall atvoss files."
        return 0
    fi
    ...
    sh "$SOURCE_INSTALL_COMMON_PARSER_FILE" --package="atvoss" --uninstall --username="$username" --usergroup="$usergroup" ${recreate_softlink_option} \
