#!/bin/sh
# Perform custom_install script for atvoss package
# -----------------------------------------------------------------------------------------------------------
...
log() {
    ...
    if [ "$log_type" = "INFO" -o "$log_type" = "WARNING" -o "$log_type" = "ERROR" ]; then
        echo -e "[Atvoss] [$cur_date] [$log_type]: $*"
    else
        echo "[Atvoss] [$cur_date] [$log_type]: $*" 1> /dev/null
    fi
    echo "[Atvoss] [$cur_date] [$log_type]: $*" >> "$logfile"
}
