...
struct AddConfig {
    ...
    using BlockOp = Atvoss::Ele::BlockBuilder<AddCompute, ArchTag>;
...
static void Run()
{
    ...
    std::vector<T1> hostInput2(8 * sizeof(T1), static_cast<T1>(1));
    ...
    // --- Step 8: 执行算子 ---
    using DeviceOp = typename AddConfig<T1, T2>::DeviceOp;
    DeviceOp deviceOp;
