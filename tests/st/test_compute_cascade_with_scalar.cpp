...
/*!
 * \file test_compute_cascade_with_scalar.cpp
 * \brief 测试compute级联表达，
 * out = (in1 + in2)/in3，其中in3为scalar
 */
...
struct ComputeTestConfig {
    struct ComputeTestCompute {
        ...
        __host_aicore__ constexpr auto Compute() const
        {
            ...
            auto in3 = Atvoss::PlaceHolder<3, T1, Atvoss::ParamUsage::IN>();
            auto out = Atvoss::PlaceHolder<4, Tensor<T2>, Atvoss::ParamUsage::OUT>();
...
static void Run()
{
    ...
    std::vector<T1> hostInput1(8 * sizeof(T1), static_cast<T1>(4));
    CHECK_ACL_RET(aclrtMemcpy(deviceIn1, 8 * sizeof(T1), hostInput1.data(), 8 * sizeof(T1), ACL_MEMCPY_HOST_TO_DEVICE));
    std::vector<T1> hostInput2(8 * sizeof(T1), static_cast<T1>(6));
    ...
    float in3 = 2;
    Atvoss::Tensor<T2> t4(deviceOutput, shapeArray, 1);
    auto arguments = Atvoss::ArgumentsBuilder{}.inputOutput(t1, t2, in3, t4).build();
