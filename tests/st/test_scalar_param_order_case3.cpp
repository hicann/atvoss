...
/*!
 * \file test_scalar_param_order_case3.cpp
 * \brief 测试Scalar场景：inputOutput传参顺序
 * case3: order: output, scalar, input
 */
...
struct ScalarParamOrderConfig {
    struct ScalarParamOrderCompute {
        ...
        {
            auto in = Atvoss::PlaceHolder<3, Tensor<T1>, Atvoss::ParamUsage::IN>();
            auto out = Atvoss::PlaceHolder<1, Tensor<T2>, Atvoss::ParamUsage::OUT>();
            auto scalar = Atvoss::PlaceHolder<2, T1, Atvoss::ParamUsage::IN>();
...
static void Run()
{
    ...
    // order: t2, scalar, t1
    auto arguments = Atvoss::ArgumentsBuilder{}.inputOutput(t2, scalar, t1).build();
