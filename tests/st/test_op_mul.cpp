...
struct MulConfig {
    ...
    using BlockOp = Atvoss::Ele::BlockBuilder<MulCompute, ArchTag>;
...
static void Run()
{
    ...
    std::vector<T1> hostInput2(8 * sizeof(T1), static_cast<T1>(4));
    ...
    // --- Step 8: 执行算子 ---
    using DeviceOp = typename MulConfig<T1, T2>::DeviceOp;
    DeviceOp deviceOp;
