...
/*!
 * \file test_op_subs_rhs.cpp
 * \brief 测试-运算符的功能，右操作数是scalar
 */
...
struct SubsRhsConfig {
    struct SubsRhsCompute {
    ...
    using BlockOp = Atvoss::Ele::BlockBuilder<SubsRhsCompute, ArchTag>;
...
static void Run()
{
    ...
    std::vector<T1> hostInput(8 * sizeof(T1), static_cast<T1>(3));
    ...
    // --- Step 8: 执行算子 ---
    using DeviceOp = typename SubsRhsConfig<T1, T2>::DeviceOp;
    DeviceOp deviceOp;
