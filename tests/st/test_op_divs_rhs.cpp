...
struct DivsRhsConfig {
    struct DivsRhsCompute {
    ...
    using BlockOp = Atvoss::Ele::BlockBuilder<DivsRhsCompute, ArchTag>;
...
static void Run()
{
    ...
    std::vector<T1> hostInput(8 * sizeof(T1), static_cast<T1>(15.0));
    ...
    // --- Step 8: 执行算子 ---
    using DeviceOp = typename DivsRhsConfig<T1, T2>::DeviceOp;
    DeviceOp deviceOp;
