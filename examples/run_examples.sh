...
ATVOS_HOME_DIR=$CURRENT_DIR/../
TEST_CASE_LIST=$(ls $ATVOS_HOME_DIR/examples | grep -v '^run_examples.sh$' | grep -v '^ops_*' | grep -v '^common*' | grep -v '^README*'  | xargs)
if [ $# -lt 1 ]; then
...
function compile_operator(){
    ...
    cd $ATVOS_HOME_DIR/examples/$TEST_NAME
    if [ -z "$RUN_MODE" ]; then
        echo "Executing with npu mode"
        bisheng -x asc --npu-arch=dav-2201 $TEST_NAME.cpp -o $TEST_NAME -I ${ATVOS_HOME_DIR}/include -I ${CURRENT_DIR}/common -ltiling_api -lplatform -lm -ldl -L${_ASCEND_INSTALL_PATH}/lib64 -w
...
if [[ " $TEST_CASE_LIST " == *" ${TEST_NAME} "* ]]; then
    ...
    echo "Error: Cannot find '$TEST_NAME' in ${ATVOS_HOME_DIR}examples. Execution example: 'bash run_examples.sh [$TEST_CASE_LIST]'"
    exit 1
fi