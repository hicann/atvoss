# 1. ATVOS概述
ATVOS针对昇腾AI处理器的Vector计算的多核并行计算过程，提供了一套统一的编程模型，可简化用户开发Vector算子的复杂度，简化Tiling计算，并支持模板的自由组合及模板扩展。
...
# 2. 公共基础工具
## 2.1 表达式模板
ATVOS使用表达式模板进行计算流的描述，表达式模板是C++利用运算符重载、类型计算来实现延迟计算的技巧。表达式模板的基本要点是在形成使用运算符或进行函数调用时，我们只是把要进行的操作记录下来不立即进行计算。直到最后Tile层计算的时候，才会进行计算操作。详细的表达式模板实现，可从[Expression表达式模板](../include/utils/expression/expression.h)进入阅读代码。

# 3. 基于ATVOS的算子开发
## 3.1 实现自定义计算表达式
ATVOS模板支持Expr的计算表达，用户通过计算表达完成对自定义算子的描述。具体使用样例如下：
```cpp
...
以上代码为例，定义一个`RmsNormOp`的Expr模板类。Tile块的切分设置，使用如下示例的固定写法，使用`AscendC::Shape`模板定义每个维度的元素大小。
```cpp
using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
using layout = AscendC::Std::tuple<shape>;
```
...
| 表达式元素| 功能描述|约束说明|
| ------------ | ------------ |---|
| `DefineParam` | 定义输入输出参数信息，传入的模板参数为<序号，Type，layout，输入输出标识>|序号需要按照`DefineParam`定义的先后顺序，从1开始累加； Type固定设置为 `VectorType<T>`， 用户无需感知；暂不支持不同的输入输出设置不同的layout；输入输出标识为可选参数，默认为`ParamUsage::in`，还可设置为`ParamUsage::out`|
| `DefineLocalVarLike` | 定义计算时需要使用的临时buffer|模板参数是标识该临时buffer内存大小跟随哪个`DefineParam`的|
...
```c++
template<typename T>
struct AddOp : ATVOS::ExprTmpl::Maker {
    using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
    using layout = AscendC::Std::tuple<shape>;
    using maxSizeType = T;
    template <template <typename> class VectorType>
    __host_aicore__ constexpr auto Get() const
    {
        ...
        auto _1 = DefineParam<1, VectorType<T>, layout>();
        auto _2 = DefineParam<2, VectorType<T>, layout>();
        auto _3 = DefineParam<3, VectorType<T>, layout, ParamUsage::out>();
        return (_3 = _1 + _2);
    }
};
```
`_1`对应`src0`，`_2`对应`src1`，`_3`对应`dst`， `count`用户无需感知。
...
```c++
template<typename T>
struct SubOp : ATVOS::ExprTmpl::Maker {
    using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
    using layout = AscendC::Std::tuple<shape>;
    using maxSizeType = T;
    template <template <typename> class VectorType>
...
```c++
template<typename T>
struct MulOp : ATVOS::ExprTmpl::Maker {
    using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
    using layout = AscendC::Std::tuple<shape>;
    using maxSizeType = T;
    template <template <typename> class VectorType>
...
```c++
template<typename T>
struct DivOp : ATVOS::ExprTmpl::Maker {
    using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
    using layout = AscendC::Std::tuple<shape>;
    using maxSizeType = T;
    template <template <typename> class VectorType>
...
__aicore__ inline void DivsAssign(AscendC::LocalTensor<T>& dst, const AscendC::LocalTensor<T>& src, uint32_t count = 0)
```
#### 3.2.5.3 模版参数说明
| 参数名         | 描述                          |
|-------------|-----------------------------|
...
```c++
template<typename T>
struct DivsOp : ATVOS::ExprTmpl::Maker {
    using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
    using layout = AscendC::Std::tuple<shape>;
    using maxSizeType = T;
    template <template <typename> class VectorType>
...
```
`_1`对应`src`，`_2`对应`dst`，`2`对应`scalarValue`，`count`用户无需感知。
...
```cpp
template <typename T>
__aicore__ inline void ExpAssign(AscendC::LocalTensor<T>& dst, const AscendC::LocalTensor<T>& src, uint32_t count = 0)
```
#### 3.2.6.3 模版参数说明
| 参数名       | 描述                  |
|-----------|---------------------|
...
```c++
template<typename T>
struct ExpOp : ATVOS::ExprTmpl::Maker {
    using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
    using layout = AscendC::Std::tuple<shape>;
    using maxSizeType = T;
    template <template <typename> class VectorType>
    __host_aicore__ constexpr auto Get() const
    {
        using shape = AscendC::Shape<Int<32>, Int<32>>;
        using layout = AscendC::Std::tuple<shape>;
        auto _1 = DefineParam<1, VectorType<T>, layout>();
        auto _2 = DefineParam<2, VectorType<T>, layout, ParamUsage::out>();
        return (_2 = Exp(_1));
...
```
`_1`对应src，`_2`对应dst，`count`用户无需感知。
...
```cpp
template <typename T>
__aicore__ inline void SqrtAssign(AscendC::LocalTensor<T>& dst, const AscendC::LocalTensor<T>& src, uint32_t count = 0)
```
#### 3.2.7.3 模版参数说明
| 参数名       | 描述                  |
|-----------|---------------------|
...
```c++
template<typename T>
struct SqrtOp : ATVOS::ExprTmpl::Maker {
    using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
    using layout = AscendC::Std::tuple<shape>;
    using maxSizeType = T;
    template <template <typename> class VectorType>
...
```cpp
template <auto scalarValue, typename T>
__aicore__ inline void PowerAssign(AscendC::LocalTensor<T>& dst, const AscendC::LocalTensor<T>& src, uint32_t count = 0)
```
#### 3.2.8.3 模版参数说明
| 参数名         | 描述                  |
|-------------|---------------------|
...
```c++
template<typename T>
struct PowerOp : ATVOS::ExprTmpl::Maker {
    using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
    using layout = AscendC::Std::tuple<shape>;
    using maxSizeType = T;
    template <template <typename> class VectorType>
...
| dst            | 输出    | 目的操作数       |
| src            | 输入    | 源操作数        |
| operationShape | 输入    | 需要广播数据的维度数据 |
#### 3.2.9.5 表达式调用示例
```c++
template<typename T>
struct BroadcastOp : ATVOS::ExprTmpl::Maker {
    using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
    using layout = AscendC::Std::tuple<shape>;
    using maxSizeType = T;
    template <template <typename> class VectorType>
    __host_aicore__ constexpr auto Get() const
    {
        auto _1 = DefineParam<1, VectorType<T>, layout>();
        auto _2 = DefineParam<2, VectorType<T>, layout, ParamUsage::out>();
        return (_2 = Broadcast<ATVOS::Patterns::Pattern::AB>(_1));
    }
};
```
`_1`对应`src`，`_2`对应`dst`，`operationShape`用户无需感知。
...
```c++
template<typename T>
struct ReduceSumOp : ATVOS::ExprTmpl::Maker {
    using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
    using layout = AscendC::Std::tuple<shape>;
    using maxSizeType = T;
    template <template <typename> class VectorType>
    __host_aicore__ constexpr auto Get() const
    {
        auto _1 = DefineParam<1, VectorType<T>, layout>();
        auto _2 = DefineParam<2, VectorType<T>, layout, ParamUsage::out>();
        return (_2 = ReduceSum<ATVOS::Patterns::Pattern::AR>(_1));
    }
};
```
`_1`对应`src`，`_2`对应`dst`，`operationShape`用户无需感知。
...
#### 3.2.11.2 函数原型
```cpp
template <CastMode castMode, typename T1, typename T2>
...
struct CasrOp : ATVOS::ExprTmpl::Maker {
    using shape = AscendC::Shape<Int<HEIGHT>, Int<WIDTH>>;
    using layout = AscendC::Std::tuple<shape>;
    ...
    __host_aicore__ constexpr auto Get() const
    {
        ...
        return (_2 = Cast<ATVOS::CastMode::CAST_ROUND>(_1));
    }
};
```
`_1`对应`src`，`_2`对应`dst`，`count`用户无需感知。

## 3.3 配置Block层模板
Block层模板负责单个核的任务分解到多个Tile的数据搬运、Tile层的计算调用。

### 3.3.1 BlockEleWise模板
```cpp
template <typename ExprMaker, const auto& Policy=policyDefault, class OpTraits = void, typename Arguments = ATVOS::Block::Config,
...
| [OpTraits](#33122-OpTraits（预留）)   | Block层输入输出配置项     |
| [Arguments](#33123-Arguments) | Block层数据元素切分结果配置项 |
| [Schedule](#33124-Schedule（预留）)   | Block层切分算法        |

#### 3.3.1.1 BlockEleWise模板的范式表达
`class BlockEleWise`模板对外提供三个部分内容：
- 对外提供的静态参数信息。
```cpp
// 算子数学表达式结构体
...
// 声明参数类型
using Params = ATVOS::ExprTmpl::Params_t<Expr>; // 萃取表达式所有参数类型列表
using InParams = ATVOS::ExprTmpl::InParams_t<Expr>; // 萃取表达式输入参数类型列表
using OutParams = ATVOS::ExprTmpl::OutParams_t<Expr>; // 萃取表达式输出参数类型列表
using LocalVars = ATVOS::ExprTmpl::LocalVars_t<Expr>; // 萃取表达式临时参数类型列表
...
static constexpr uint32_t BASIC_BLOCK = ATVOS::Tile::GetTotal<0, ExpressMaker>();
```
- 默认构造函数
```cpp
// 默认构造函数：根据表达式最大存活节点中输入、输出元素个数进行UB内存分配的管理，主要实现在utils/buf_pool/loopbuf.h中
_aicore__ inline BlockEleWise();
```
...
```
- 编译期计算Block层配置，作为默认[Arguments](#33123-Arguments)参数
```cpp
// Block层默认配置计算
...
struct PolicyEleWise {
    ...
    uint32_t shapeNValue = 0;         // 0: 不需要额外对齐， 其他场景： 按这个配置值对齐
};
```
#### 3.3.1.2.2 OpTraits（预留）
`OpTraits`：当前层的输入输出的类型信息。
#### 3.3.1.2.3 Arguments
`Arguments`：根据不同策略计算出来的`Tile`层处理的元素信息。
```cpp
// Block层的元素切分信息
struct Config {
...
```
#### 3.3.1.2.4 Schedule（预留）
`Schedule`：多Tile切分算法类实际计算。

#### 3.3.1.3 模板对外接口说明
#### 3.3.1.3.1 Run接口
`BlockEleWise`提供`Run()`接口按批次完成输入数据从GM->UB，计算，输出结果UB->GM过程，参数和函数说明如下：
...
`BlockEleWise`提供`MakeBlockParam()`接口，参数和函数说明如下：
- 参数说明：用户输入`Block`层承载切分信息的`blockParam`。
- 函数功能说明：`Block`层计算切分参数后并填充`blockParam`。
```cpp
static constexpr bool MakeBlockParam(Arguments& blockParam)
{
...
当前实现了KernelEleWise模板，其具备对逐元素操作进行高效多核任务分配与调度的能力。
### 3.4.1 KernelEleWise模板
...
```
#### 3.4.1.1 KernelEleWise模板的范式表达
支持用户自定义修改模板参数实现，实现局部拓展，也支持用户按照规则自定义模板类，实现模板拓展。
`KernelEleWise`类定义如下：
```cpp
template <typename BlockOp, const auto &Policy = policyDefault, class OpTraits = void, typename Arguments = ATVOS::Kernel::Config, class Schedule = DefaultSchedule<BlockOp, Policy, OpTraits, Arguments>>
class KernelEleWise {
    ...
}
```
KernelEleWise模板对外提供五部分内容：
- 对外提供的模板参数信息  
...
当前`KernelEleWise`提供默认的配置策略`PolicyEleWise`，结构体定义如下：
```cpp
...
- 默认模板构造函数
```cpp
DeviceAdapter(std::vector<uint32_t> &shapeInfo);
```
`shapeInfo`是用户定义数据的各个维度的信息。

- 对外运行接口
接口固定命名`Run()`，接口接受不定长参数`template <typename... Args>`
```cpp
template <typename... Args>
int64_t Run(Args&&... args);
```
在使用时，限制Run的入参必须是`std::vector`， 并且入参需要严格和`KernelOp::ExprMaker`计算表达中描述的算子输入输出信息保持一致（参数个数、顺序），注意只包含使用`DefineParam`类型定义的输入输出信息。
...
DeviceOp deviceOp;
deviceOp.Run(v1, v2, v3); // 入参需要与KernelOp::ExprMaker定义的匹配
```
