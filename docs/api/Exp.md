...
```Cpp
template<typename T>
struct OpExp : UnaryOp<T>

template<typename T>
__host_aicore__ constexpr auto Exp(Expression<T> lhs)

template<typename T>
__host_aicore__ constexpr auto Exp(T &&lhs)
```
## 参数说明
...
<tbody>
  ...
  <tr>
    ...
    <td>输入</td>
    <td>NA</td>
    <td>Exp左操作数，当类型是Expression&lt;T&gt;时，是张量，当类型是T时，是标量</td>
    <td>NA</td>
  </tr>
</tbody>
</table>
